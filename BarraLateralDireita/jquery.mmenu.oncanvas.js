/* Custom changes for Mega Menu 2018 */
...
//			preventDefault		: null,
	...
	$[ _PLUGIN_ ].prototype = {
					...
					function( i )
					{
						...
						if ( $l.parent().is( '.' + _c.list ) )
						{
							...
							if ( that.opts.slidingSubmenus )
							{
								...
								//Section text
								$u.prepend( '<li class="' + _c.subtitle + '"><a class="' + _c.subclose + '" href="#' + $p.attr( 'id' ) + '">' + $a[0].textContent + '</a></li>' );
							}
						}
					}
				);
		...
		_openSubmenuHorizontal: function( $opening )
		{
			...
			setTimeout(
				function()
				{
					...
					//Force resize of mobile carousel
		...
		_resizeCarousel: function(){
			...
			jQuery('#mobile-menu .slick-slider').each(function() {
				jQuery(this).slick("getSlick").resize();
				jQuery(this).slick("getSlick").setPosition();
			});
		},
	...
	*/
	...
	function _initPlugin()
	{
		...
		_c.add( 'rightwrapper menu inline panel nopanel list nolist subtitle selected label spacer current highest hidden opened subopened subopen fullsubopen subclose' );
		_c.umm = function( c )
		{
			if ( c.slice( 0, 3 ) == 'mm-' )
			{
