...
var Class = function() {
    ...
    function create() {
        ...
        for (var i =
                0, length = properties.length; i < length; i++) klass.addMethods(properties[i]);
...
(function() {
    var _toString = Object.prototype.toString,
        ...
        NATIVE_JSON_STRINGIFY_SUPPORT = window.JSON && typeof JSON.stringify === "function" && JSON.stringify(0) ===
    ...
    function Str(key, holder, stack) {
        ...
        if (Type(value) === OBJECT_TYPE && typeof value.toJSON === "function") value =
            value.toJSON(key);
...
Object.extend(Function.prototype, function() {
    ...
    if (!Function.prototype.bind) extensions.bind =
        bind;
    return extensions
}());
(function(proto) {
...
Object.extend(String.prototype, function() {
    ...
    function inspect(useDoubleQuotes) {
        var escapedString = this.replace(/[\x00-\x1f\\]/g, function(character) {
            ...
            return "\\u00" + character.charCodeAt().toPaddedString(2,
                16)
        });
...
var Enumerable = function() {
    ...
    function grep(filter, iterator, context) {
        ...
        this.each(function(value, index) {
            if (filter.match(value)) results.push(iterator.call(context,
                value, index, this))
        }, this);
        return results
    }
    ...
    function inject(memo, iterator, context) {
        this.each(function(value, index) {
            memo =
                iterator.call(context, memo, value, index, this)
        }, this);
        return memo
    }
...
(function() {
    ...
    function some(iterator) {
        ...
        var object = Object(this);
        for (var i = 0, length = object.length >>> 0; i < length; i++)
            if (i in object && iterator.call(context, object[i], i, object)) return true;
        return false
    }
    ...
    function every(iterator) {
        ...
        var object = Object(this);
        for (var i = 0, length = object.length >>> 0; i < length; i++)
            if (i in object && !iterator.call(context, object[i], i, object)) return false;
        return true
    }
    ...
    if (!arrayProto.reduce) var inject =
        Enumerable.inject;
...
var Hash = Class.create(Enumerable, function() {
    ...
    function toQueryString() {
        return this.inject([], function(results, pair) {
            ...
            } else results.push(toQueryPair(key,
                values));
            return results
        }).join("&")
    }
    ...
    respondToReadyState: function(readyState) {
        ...
        if (state == "Complete") {
            try {
                this._complete = true;
                (this.options["on" +
                    response.status] || this.options["on" + (this.success() ? "Success" : "Failure")] || Prototype.emptyFunction)(response, response.headerJSON)
            } catch (e) {
                this.dispatchException(e)
            }
...
(function(GLOBAL) {
    ...
    function Element(tagName, attributes) {
        ...
        var node = shouldUseCreationCache(tagName, attributes) ? ELEMENT_CACHE[tagName].cloneNode(false) :
    ...
    function insertContentAt(element, content, position) {
        ...
        for (var i = 0, node; node = childNodes[i]; i++) method(element,
            node);
        content.evalScripts.bind(content).defer()
    }
        ...
        if (!HAS_UNIQUE_ID_PROPERTY) {
            ...
            if (deep) {
                var descendants = Element.select(clone, "*"),
                    i = descendants.length;
                while (i--) descendants[i]._prototypeUID =
                    UNDEFINED
            }
        }
    ...
    function identify(element) {
        ...
        Element.writeAttribute(element,
            "id", id);
        return id
    }
    ...
    function normalizeStyleName_IE(style) {
        if (style ===
            "float" || style === "cssFloat") return "styleFloat";
        return style.camelize()
    }
    ...
    function setStyle(element, styles) {
        ...
        for (var property in styles)
            if (property === "opacity") Element.setOpacity(element, styles[property]);
                ...
                if (property === "float" ||
                    property === "cssFloat") property = Object.isUndefined(elementStyle.styleFloat) ? "cssFloat" : "styleFloat";
    ...
    function getStyle_IE(element, style) {
        ...
        if (value === "auto") {
            if ((style === "width" || style ===
                    "height") && Element.visible(element)) return Element.measure(element, style) + "px";
            return null
        }
        return value
    }
    ...
    var HAS_UNIQUE_ID_PROPERTY = "uniqueID" in
        DIV;
    ...
    function extendElementWith(element, methods) {
        for (var property in methods) {
            ...
            if (Object.isFunction(value) && !(property in
    ...
    function extend(element) {
        ...
        EXTENDED[getUniqueElementID(element)] =
            true;
        return element
    }
...
(function() {
    ...
    function getOffsetParent(element) {
        ...
        while ((element = element.parentNode) && element !== document.body)
            if (Element.getStyle(element, "position") !== "static") return isHtml(element) ?
        ...
        do
            if (element === document.body) {
                ...
                valueL += !Object.isUndefined(window.pageXOffset) ?
                    window.pageXOffset : bodyScrollNode.scrollLeft || 0;
                break
            } else {
    ...
    function makePositioned(element) {
        ...
        if (position === "static" || !position) {
            ...
            Element.store(element, "prototype_made_positioned",
                true)
        }
        return element
    }
    ...
    function makeClipping(element) {
        ...
        if (Object.isUndefined(madeClipping)) {
            var overflow =
                Element.getStyle(element, "overflow");
    ...
    function clonePosition(element, source, options) {
        ...
        if (options.setLeft) styles.left = p[0] - delta[0] +
...
Prototype.Selector = function() {
    ...
    function extendElements(elements) {
        for (var i = 0, length = elements.length; i <
            length; i++) Element.extend(elements[i]);
        return elements
    }
    var K = Prototype.K;
    return {
...
(function(window) {
    ...
    setDocument = Sizzle.setDocument = function(node) {
        ...
        if (parent && parent !== parent.top)
            if (parent.addEventListener) parent.addEventListener("unload",
                function() {
                    setDocument()
                }, false);
        ...
        if (support.qsa = rnative.test(doc.querySelectorAll)) {
            ...
            assert(function(div) {
                ...
                if (!div.querySelectorAll(":enabled").length) rbuggyQSA.push(":enabled",
                    ":disabled");
    ...
    Expr = Sizzle.selectors = {
        ...
        preFilter: {
            ATTR: function(match) {
                match[1] =
                    match[1].replace(runescape, funescape);
        ...
        filter: {
                ...
                return first === 1 && last === 0 ? function(elem) {
                    ...
                    if (parent) {
                        ...
                        if (forward && useCache) {
                            ...
                            (cache = (elem[expando] || (elem[expando] = {}))[type]) && cache[0] === dirruns) diff = cache[1];
                        else
                            while (node = ++nodeIndex && node && node[dir] || (diff = nodeIndex = 0) || start.pop())
            ...
            PSEUDO: function(pseudo, argument) {
                var args, fn = Expr.pseudos[pseudo] || Expr.setFilters[pseudo.toLowerCase()] || Sizzle.error("unsupported pseudo: " +
                    pseudo);
    ...
    function setMatcher(preFilter,
        selector, matcher, postFilter, postFinder, postSelector) {
            ...
            superMatcher = function(seed, context, xml, results, outermost) {
                ...
                if (bySet && i !== matchedCount) {
                    ...
                    if (outermost && !seed && setMatched.length >
                        0 && matchedCount + setMatchers.length > 1) Sizzle.uniqueSort(results)
                }
                if (outermost) {
...
Form.Element.Serializers = function() {
    ...
    function select(element, value) {
        if (Object.isUndefined(value)) return (element.type ===
...
(function(GLOBAL) {
    ...
    if (window.attachEvent)
        if (window.addEventListener) isIELegacyEvent = function(event) {
    ...
    if (window.attachEvent)
        if (!window.addEventListener) _isButton = _isButtonForLegacyEvents;
    ...
    function observe(element, eventName, handler) {
        ...
        if (isCustomEvent(eventName)) observeCustomEvent(element,
            eventName, responder);
    ...
    function observeCustomEvent(element, eventName, responder) {
        if (element.addEventListener) element.addEventListener("dataavailable", responder, false);
            ...
            element.attachEvent("ondataavailable",
                responder);
...
(function(GLOBAL) {
    ...
    function createResponder(uid, eventName, handler) {
        ...
        if (isSimulatedMouseEnterLeaveEvent(eventName)) return createMouseEnterLeaveResponder(uid,
            eventName, handler);
        return function(event) {
...
var Position = {
    ...
    overlap: function(mode, element) {
        if (!mode) return 0;
        if (mode ==
            "vertical") return (this.offset[1] + element.offsetHeight - this.ycomp) / element.offsetHeight;
